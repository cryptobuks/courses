 ...
 * @public
 */
export const initialize = (app) => app.hooks
    .fromMap({
    'feature.clean': true,
    'feature.hash': false,
    ...
    'build.module.rules.oneOf': () => Object.values(app.build.rules).map(rule => rule.toWebpack()),
    ...
    'build.recordsPath': () => app.path(`@storage`, app.label, `modules.json`),
    ...
    'build.target': () => app.context.manifest?.browserslist
    ...
    .when(app.isDevelopment, ({ hooks }) => hooks.fromMap({
    'dev.middleware.dev.options.writeToDisk': true,
    'dev.middleware.dev.options.publicPath': () => app.hooks.filter(`build.output.publicPath`),
