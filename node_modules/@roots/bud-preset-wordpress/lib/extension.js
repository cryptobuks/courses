...
let BudPresetWordPress = class BudPresetWordPress extends Extension {
    ...
    async configAfter() {
        if (!this.app.isDevelopment)
            return;
        if (this.app.api.trace.some(([method]) => method === `proxy`))
            return;
        if (this.app.hooks.filter(`dev.middleware.proxy.target`))
            return;
