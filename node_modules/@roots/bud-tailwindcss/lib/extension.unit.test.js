...
describe(`@roots/bud-tailwindcss extension`, () => {
    ...
    it(`basic checks`, () => {
        ...
        expect(extension.options.generateImports).toBe(extension.getOption(`generateImports`));
    });
    ...
    it(`should not call extensions.add when generateImports is false`, async () => {
        ...
        extension.app.extensions.add = jest.fn();
    ...
    it(`should return a copy of the resolved config`, async () => {
        const extension = new Extension(Bud);
        const configInitial = resolveConfig(Bud.context.config[`tailwind.config.js`].module);
        expect(extension.theme.colors).not.toBe(configInitial.theme.colors);
    });
    it(`should have a config prop`, async () => {
        const extension = new Extension(Bud);
        expect(extension.config.module).toBe(Bud.context.config[`tailwind.config.js`].module);
    });
    ...
    it(`should throw when key does not exist`, async () => {
        const extension = new Extension(Bud);
        try {
            expect(() => extension.resolveThemeValue(`foo`)).toThrowErrorMatchingSnapshot();
        }
        catch (e) { }
    });
    ...
    it(`should throw when key does not exist in extended config (filter extends)`, async () => {
        const extension = new Extension(Bud);
        try {
            expect(() => extension.resolveThemeValue(`lineHeight`, true)).toThrowErrorMatchingSnapshot();
        }
        catch (e) { }
    });
    ...
    it(`should register alias when generateImports is not undefined`, async () => {
        ...
        expect(Bud.hooks.async).toHaveBeenCalledWith(`build.resolve.alias`, expect.any(Function));
    });
