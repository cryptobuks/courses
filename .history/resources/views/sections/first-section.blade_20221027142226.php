...
<section class="mx-auto p-4 max-w-pageWidth">
    <div class="w-full  m-auto">
        <h3 class="w-full justify-items-center !font-bold text-center p-4 text-l md:!text-6xl">{{$heading}}</h3>
        <h1 class="w-full justify-items-center !font-normal text-white text-center mb-4 !text-2xl">{{$subheading}}</h1>
        <h3 class="w-full justify-items-center font-black text-center p-2">{{$thirdTitle}}</h3>
    </div>
    <div class="flex flex-row mx-auto">
        <div class="flex-auto  relative !md:w-full w-1/2">
            <img class="rounded w-full relative" src="{{$expertMainPhoto}}">
            </img>
            @include('partials.expert-info')
        </div>
        <div class="flex-auto ml-2 border-4 border-primary rounded border-t-40 !md:w-full w-1/2">
