<button id="moving-buy-button" class="bg-primary w-full rounded-xl md:rounded-full p-8 text-center block md:flex justify-center text-black align-center drop-shadow-md shadow-primary  ">
    <?php
        ...
        <?php if($price_sales > 0){
            echo "<div class='md:text-3xl text-xl mr-2 ml-2 font-semibold !no-underline uppercase'> Join the challange " . "$" ."</div>" . $price_sales . "&nbsp" . "</div>" . "<div class='text-xl mr-2 ml-2 font-medium line-through pt-1'>" . "$" . $price . "</div>";
        }
        else{
            echo $price . "$";
        }?>
