    ...
    <a href="?wffn-next-link=yes">
        <button id="moving-buy-button" class="bg-primary w-full rounded-3xl p-8 text-center block md:flex justify-center text-black align-center drop-shadow-md shadow-primary  ">
            <?php
                ...
                if($price_sales > 0){
                    echo "<div class='text-2xl mr-2 ml-2 font-semibold !no-underline'> Join the challange " . "$"  . $price_sales . "&nbsp" . "</div>" . "<div class='text-xl mr-2 ml-2 font-medium line-through'>" . "$" . $price . "</div>";
                }
                else{
                    echo $price . "$";
                }
