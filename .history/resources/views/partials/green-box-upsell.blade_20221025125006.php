<section class="container mx-auto p-4 flex justify-center ">
    <div class="width-780 border-dashed border-2 border-green-600 rounded p-8 bg-green-100">
        <div>{{$greenBoxTitle}}</div>
        <div>{{$greenBoxNoThanks}}</div>
        <?php
	$p = get_field('product_upsale');
	...
	echo "Today " . "<span class='date'>" . date("d.m.Y"). "</span>" . "  we are offering you a " . "<span class='date'>" . $format_number1 . "% " . "</span>" . "discount. Take advantage of this offer before time runs out.";
?>
