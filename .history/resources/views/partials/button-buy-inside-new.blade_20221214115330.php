<button id="moving-buy-button" class="relative block  md:w-1/2  text-center cursor-pointer" onclick="toggleModal('modal-id')">
      ...
      <?php
        ...
        <div class="flex flex-row">
                <div class="h-6 mt-1 whitespace-nowrap text-white">@include('icons.shopping-cart')</div>
            ...
            <?php if($price_sales > 0){
                echo "<div class='flex md:text-3xl text-xl mr-2 ml-2 font-semibold !no-underline uppercase text-white items-center'>" .  "$"  . $price_sales . "&nbsp" . "<div class='text-xl mr-2 ml-2 font-semibold line-through pt-1'>" . "$" . $price . "</div>" . "</div>" ;
            }
        else{
            echo $price . "$";
        }?>
